...
const (
	...
	// Message means that `from` is tombstone. Leader can then update
	// removed_records.
...
type RaftMessage struct {
	...
	// If it has value, to_peer should be removed if merge is never going to
	// complete.
...
type RegionLocalState struct {
	...
	// Merged peer can't be deleted like gc peers. Instead, leader needs to query
	// target peer to decide whether source peer can be destroyed.
...
type ExtraMessage struct {
	...
	// It's merge related index. In `WantRollbackMerge`, it's prepare merge index.
	// In `MsgGcPeerRequest`, it's the commit merge index. In
	// `MsgVoterReplicatedIndexRequest` it's the voter_replicated_index.
	Index uint64 `protobuf:"varint,2,opt,name=index,proto3" json:"index,omitempty"`
	// In `MsgCheckStalePeerResponse`, it's the peers that receiver can continue
	// to query.
